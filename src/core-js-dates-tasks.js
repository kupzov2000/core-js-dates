...
function getTime(date) {
  const dateCurr = date;
  return `${String(dateCurr.getHours()).padStart(2, '0')}:${String(dateCurr.getMinutes()).padStart(2, '0')}:${String(dateCurr.getSeconds()).padStart(2, '0')}`;
}
...
function getDayName(date) {
  ...
  return day[new Date(formattedDate).getDay()];
}
...
function getNextFriday(date) {
  const dateNumb = new Date(date).getDay();
  const dateResult = new Date(date);

  if (dateNumb === 5) {
    dateResult.setDate(dateResult.getDate() + 7);
  } else if (dateNumb < 5) {
    dateResult.setDate(dateResult.getDate() + (5 - dateNumb));
  } else {
    dateResult.setDate(dateResult.getDate() + 6);
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const resultDifference =
    (new Date(dateEnd) - new Date(dateStart)) / 86400000 + 1;
...
function getCountWeekendsInMonth(month, year) {
  const totalDayMonth = new Date(year, month, 0).getDate();
  let count = 0;
  for (let i = 1; i <= totalDayMonth; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const firstDayWeek = firstJan.getDay() || 7;
  const diff = (date - firstJan) / (24 * 60 * 60 * 1000);
  const weekNumb = Math.ceil((diff + firstDayWeek) / 7);
...
function getNextFridayThe13th(date) {
  ...
  const month = day <= 13 ? date.getMonth() : date.getMonth() + 1;
  let result;

  for (let i = month; i <= 12; i += 1) {
    const frid13th = new Date(year, i, 13);
    if (frid13th.getDay() === 5) {
      result = frid13th;
      break;
    }
  }
